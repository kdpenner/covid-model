...
def get_patient_data(max_delay=60):
    ...
    """
    url = ("https://github.com/beoutbreakprepared/nCoV2019/raw/master/"
    ...
    def is_ten_char(x):
        return x.str.len().eq(10)

    patients = patients[
      is_ten_char(patients.Confirmed) & is_ten_char(patients.Onset)]

    # Convert both to datetimes
    patients.Confirmed = pd.to_datetime(
      patients.Confirmed, format="%d.%m.%Y", errors="coerce")
    patients.Onset = pd.to_datetime(patients.Onset, format="%d.%m.%Y",
                                    errors="coerce")
    ...
    patients = patients[
      patients.Onset < patients.Onset.max() - pd.Timedelta(days=14)]
...
def get_emp_delay_distribution():
    ...
    try:
        ...
        p_delay /= p_delay.sum()
        p_delay = (
            pd.Series(np.zeros(incubation_days)).append(p_delay,
                                                        ignore_index=True))
        p_delay.rename("p_delay")
        p_delay.to_csv(os.path.expanduser("~/.local/share/rtlive/p_delay.csv"),
                       index=False)
